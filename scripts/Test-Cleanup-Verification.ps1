<#
.SYNOPSIS
    驗證 Vencord Auto Patcher 暫存檔自動清理機制
#>

$tempDir = Join-Path $env:LOCALAPPDATA "VencordAutoPatcher\temp"

Write-Host "==========================================================" -ForegroundColor Cyan
Write-Host "  Vencord Auto Patcher 暫存檔無痕清理驗證工具" -ForegroundColor Cyan
Write-Host "==========================================================" -ForegroundColor Cyan
...
$exePath = Join-Path $PSScriptRoot "..\bin\VencordAutoPatcher.exe"
if (Test-Path $exePath) {
    & $exePath --force --no-launch
} else {
    & (Join-Path $PSScriptRoot "..\VencordAutoPatcher.ps1") -Force -NoLaunch
}
